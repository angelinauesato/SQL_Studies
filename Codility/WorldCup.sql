...
WITH HOST_GAMES AS (
    ...
    SELECT host_team AS team_id,
       CASE WHEN coalesce(host_goals,0,host_goals) > coalesce(guest_goals,0,guest_goals) THEN 3
            WHEN coalesce(host_goals,0,host_goals) = coalesce(guest_goals,0,guest_goals) THEN 1
            ELSE 0
            END AS Points
FROM Matches
...
TOTAL_POINTS AS(
    SELECT team_id, SUM(Points) AS points FROM(
        SELECT team_id, points FROM HOST_GAMES
        UNION ALL
        SELECT team_id, points FROM GUEST_GAMES
    ) total
    GROUP BY team_id
),
...
ALL_TEAMS_MATCH AS(
    SELECT t.team_id, t.team_name, 
            SUM(coalesce(tp.points, 0,tp.points) ) AS num_points
    FROM 
    Teams t
    INNER JOIN
    TOTAL_POINTS tp ON t.team_id = tp.team_id 
...
TEAM_NOT_PLAY AS(
    SELECT t.team_id, t.team_name, SUM(0) AS num_points
    FROM Teams t 
    WHERE t.team_id NOT IN (SELECT host_team FROM Matches) AND  
    t.team_id NOT IN (SELECT guest_team FROM Matches)
    GROUP BY t.team_id, t.team_name
)

SELECT team_id, team_name, num_points FROM(
    SELECT team_id, team_name, num_points FROM ALL_TEAMS_MATCH
    UNION ALL

    SELECT team_id, team_name, num_points FROM TEAM_NOT_PLAY
) x
